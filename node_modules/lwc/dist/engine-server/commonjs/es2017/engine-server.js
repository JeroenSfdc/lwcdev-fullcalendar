...
function isAriaAttribute(attrName) {
  return attrName in AriaAttrNameToPropNameMap$1;
}
/*
 * Copyright (c) 2018, salesforce.com, inc.
 * All rights reserved.
...
if (typeof Event !== 'function' && typeof CustomEvent !== 'function') {
  ...
  defineProperties$2(_globalThis$1, {
    Event: {
      value: Event,
      configurable: true,
      writable: true
...
const BaseBridgeElement = HTMLBridgeElementFactory(HTMLElementConstructor, getOwnPropertyNames$2(HTMLElementOriginalDescriptors), []);
...
const renderer = {
  ...
  insert(node, parent, anchor) {
    ...
    node.parent = parent;
    const anchorIndex = isNull$1(anchor) ? -1 : parent.children.indexOf(anchor);
  ...
  getProperty(node, key) {
    ...
    if (node.type === HostNodeType.Element) {
      const attrName = htmlPropertyToAttribute$1(key); // Handle all the boolean properties.
  ...
  setProperty(node, key, value) {
    ...
    if (node.type === HostNodeType.Element) {
      ...
      if (node.name === 'input' && attrName === 'value') {
        return isNull$1(value) || isUndefined$4(value) ? this.removeAttribute(node, 'value') : this.setAttribute(node, 'value', value);
      }
    }
  ...
  getClassList(element) {
    function getClassAttribute() {
      let classAttribute = element.attributes.find(attr => attr.name === 'class' && isNull$1(attr.namespace));

      if (isUndefined$4(classAttribute)) {
        classAttribute = {
          name: 'class',
          namespace: null,
          value: ''
  ...
  setCSSStyleProperty(element, name, value) {
    let styleAttribute = element.attributes.find(attr => attr.name === 'style' && isNull$1(attr.namespace));

    if (isUndefined$4(styleAttribute)) {
      styleAttribute = {
        name: 'style',
        namespace: null,
        value: ''
...
function renderComponent(tagName, Ctor, props = {}) {
  ...
  if (!isObject$1(props) || isNull$1(props)) {
