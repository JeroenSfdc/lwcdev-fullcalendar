...
const hook = async function (opts) {
    ...
    const readableSuggestion = core_1.toConfiguredId(suggestion, this.config);
    const originalCmd = core_1.toConfiguredId(opts.id, this.config);
    this.warn(`${color_1.color.yellow(originalCmd)} is not a ${opts.config.bin} command.`);
    let response = '';
    try {
        response = await cli_ux_1.cli.prompt(`Did you mean ${color_1.color.blueBright(readableSuggestion)}? [y/n]`, { timeout: 4900 });
    }
    catch (error) {
        this.log('');
        this.debug(error);
