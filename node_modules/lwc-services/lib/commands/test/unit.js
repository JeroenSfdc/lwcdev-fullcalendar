...
class Test extends command_1.Command {
    async run() {
        ...
        if (!fs_1.existsSync('jest.config.js') && !fs_1.existsSync('jest.config.json')) {
            ...
            if (fs_1.existsSync('jest.config.js')) {
                const jestCustomConfigJs = require(path_1.resolve(process.cwd(), 'jest.config.js'));
                jestFinalConfig = merge(jestFinalConfig, jestCustomConfigJs);
            }
            if (fs_1.existsSync('jest.config.json')) {
                const jestCustomConfigJson = require(path_1.resolve(process.cwd(), 'jest.config.json'));
                jestFinalConfig = {
                    ...jestFinalConfig,
