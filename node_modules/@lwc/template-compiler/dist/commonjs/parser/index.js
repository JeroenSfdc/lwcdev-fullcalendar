...
function parse(source, state) {
    ...
    let parent;
    const stack = [];
    const preserveComments = templateRoot.attrs.some(({ name }) => name === constants_1.ROOT_TEMPLATE_DIRECTIVES.PRESERVE_COMMENTS) || state.config.preserveHtmlComments;
    const renderModeAttribute = templateRoot.attrs.find(({ name }) => name === constants_1.ROOT_TEMPLATE_DIRECTIVES.RENDER_MODE);
    if (renderModeAttribute) {
        const renderMode = renderModeAttribute.value;
        if (types_1.isLWCDirectiveRenderMode(renderMode)) {
            state.renderMode = renderMode;
        }
        else {
            const possibleValues = Object.values(types_1.LWCDirectiveRenderMode)
                .map((value) => `"${value}"`)
                .join(', or ');
            warnOnElement(errors_1.ParserDiagnostics.LWC_RENDER_MODE_INVALID_VALUE, templateRoot, [
                possibleValues,
            ]);
        }
    }
        ...
        Comment: {
            enter(node) {
                if (preserveComments) {
                    ...
                    const commentNode = ir_1.createComment(node, html_1.decodeTextContent(rawComment));
    ...
    function applyLwcDirectives(element) {
        ...
        if (element.parent === undefined && constants_1.ROOT_TEMPLATE_DIRECTIVES_SET.has(lwcAttribute.name)) {
            return;
        }
    ...
    function applyAttributes(element) {
        ...
        if (!state.shouldScopeFragmentId && (((_a = element.props) === null || _a === void 0 ? void 0 : _a.id) || ((_b = element.attrs) === null || _b === void 0 ? void 0 : _b.id))) {
